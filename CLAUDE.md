# Locations & Settlements (acks-location)
...
- `scripts/` — ESM runtime, entry `scripts/module.mjs`; `templates/` — .hbs;
  `styles/`; `lang/en.json` — flat i18n keys prefixed `ACKS-LOCATION.`
...
- `tools/` — dev harness. `build-packs.mjs` and `validate.mjs` are **synced
  ...
  module-owned.
- Canonical ACKS II rules extract: `C:\Proj\acks-rules\acks-location\RULES.md`
...
- Foundry dev install (junction, not copy):
  `New-Item -ItemType Junction -Path "$env:LOCALAPPDATA\FoundryVTT\Data\modules\acks-location" -Target "C:\Proj\acks-location"`
## Live testing
...
5. Confirm publication with BOUNDED polls — **never `gh run watch`, it hangs**:
   ...
   verify `https://github.com/NocTempre/acks-location/releases/latest/download/module.json`
...
- Namespacing (validate-enforced): globals/custom hooks/HB helpers start with
  ...
  `flags["acks-location"].idPrefix` key; lang keys with `ACKS-LOCATION.`;
  CSS classes with `acks-location-`.
- Design doctrine: **reuse → extend → enhance → invent** — reuse core system
  documents; extend only via `flags["acks-location"]`; enhance with alternate
